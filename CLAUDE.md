...
- 목숨은 3개(`LIVES`, 요청서 그대로). 한 판 38.5초 — README 4장.
...
**난이도를 speed 로 올리지 마라.** `lure`(0.40 → 0.68)와 `beat`(8 → 3 프레임)로 올린다.
`beat` 는 도형 사이의 틈이자 **다음 기준을 미리 읽는 시간**이라, 줄어들수록
"지금 판정 + 다음 준비"로 주의가 쪼개진다. 지각이 아니라 그쪽을 압박한다.
